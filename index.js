...
app.get("/recepie-delete/:id", function(req, res) {

  if(req.query.id) {
    console.log("Deleting : " + req.query.id);
    stockService.deleterecepie(req.query.id);
    res.status(200).send({});
  } else {
    res.status(400).send("Scuccessfully Deleted the item");
  }
});
//app.get("/recepie-delete/:id", (req, res) => {
  ...
  //});
  // console.log(foundRecepie);
  //res.render("recepies/", { foundRecepie });
//});
